...
/obj/structure/dropship_equipment/sentry_holder/proc/deploy_sentry()
	...
	else if(linked_shuttle.id == DROPSHIP_WACO)
		deployed_turret.linked_cam.network = list(CAMERA_NET_WACO)
	else if(linked_shuttle.id == DROPSHIP_ATTU)
		deployed_turret.linked_cam.network = list(CAMERA_NET_ATTU)
	if (linked_shuttle.id == DROPSHIP_SAIGON)
		deployed_turret.linked_cam.network = list(CAMERA_NET_SAIGON)
	else if (linked_shuttle.id == DROPSHIP_ARGONNE)
		deployed_turret.linked_cam.network = list(CAMERA_NET_ARGONNE)
	else if(linked_shuttle.id == DROPSHIP_MIDWAY)
		deployed_turret.linked_cam.network = list(CAMERA_NET_MIDWAY)
	if (linked_shuttle.id == DROPSHIP_TARAWA)
		deployed_turret.linked_cam.network = list(CAMERA_NET_TARAWA)
	else if (linked_shuttle.id == DROPSHIP_CONCORD)
		deployed_turret.linked_cam.network = list(CAMERA_NET_CONCORD)
